...
async function diff(targetApp, downstreamApp, githubToken, herokuUserAgent) {
    ...
    try {
        ...
        cli_ux_1.default.styledHeader(`${color_1.default.app(targetApp.name)} is ahead of ${color_1.default.app(downstreamApp.name)} by ${githubDiff.ahead_by} commit${githubDiff.ahead_by === 1 ? '' : 's'}`);
        ...
        cli_ux_1.default.log('are you sure you have pushed your latest commits to GitHub?');
    }
}
class PipelinesDiff extends command_1.Command {
    constructor() {
        ...
        this.getAppInfo = async (appName, appId) => {
            ...
            let slug;
            try {
                const { body: releases } = await api_1.getReleases(this.heroku, appId);
    ...
    async run() {
        ...
        const coupling = await api_1.getCoupling(this.heroku, targetAppName)
            .then(res => res.body)
            .catch(() => undefined);
        if (!coupling) {
            cli_ux_1.default.error(`This app (${targetAppName}) does not seem to be a part of any pipeline`);
            return;
        }
        const targetAppId = coupling.app.id;
        cli_ux_1.default.action.start('Fetching apps from pipeline');
        const allApps = await api_1.listPipelineApps(this.heroku, coupling.pipeline.id);
        cli_ux_1.default.action.stop();
        const sourceStage = coupling.stage;
        if (!sourceStage) {
            return cli_ux_1.default.error(`Unable to diff ${targetAppName}`);
        }
        ...
        if (downstreamApps.length === 0) {
            return cli_ux_1.default.error(`Cannot diff ${targetAppName} as there are no downstream apps configured`);
        }
        ...
        cli_ux_1.default.action.start('Fetching release info for all apps');
        ...
        if (!targetAppInfo.repo) {
            ...
            return cli_ux_1.default.error(`${targetAppName} does not seem to be connected to GitHub!\nRun ${color_1.default.cyan(command)} and "Connect to GitHub".`);
        }
        if (!targetAppInfo.hash) {
            return cli_ux_1.default.error(`No release was found for ${targetAppName}, unable to diff`);
        }
