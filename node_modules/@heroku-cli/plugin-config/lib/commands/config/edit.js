...
function configToString(config) {
    return Object.keys(config)
        .sort()
        .map(key => {
        return `${key}=${quote_1.quote(config[key])}`;
    })
        .join('\n');
}
...
function stringToConfig(s) {
    return s.split('\n').reduce((config, line) => {
        ...
        if (i === -1)
            error();
        config[line.slice(0, i)] = quote_1.parse(line.slice(i + 1)) || '';
        return config;
    }, {});
}
...
function showDiff(from, to) {
    for (const k of allKeys(from, to)) {
        if (from[k] === to[k])
            continue;
        if (k in from) {
            cli_ux_1.cli.log(color_1.color.red(`- ${k}=${quote_1.quote(from[k])}`));
        }
        if (k in to) {
            cli_ux_1.cli.log(color_1.color.green(`+ ${k}=${quote_1.quote(to[k])}`));
        }
    }
}
